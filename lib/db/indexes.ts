...
export async function listIndexes(
  ...
  const result = await db.query<Record<string, unknown>>(
    ...
    SELECT
      ...
      t.relname AS table_name, i.relname AS index_name, pg_get_indexdef(i.oid) AS index_definition,
      ...
      ARRAY(
        SELECT pg_get_indexdef(i.oid, position, true)
    ...
    ORDER BY pg_relation_size(i.oid) DESC, tn.nspname, t.relname, i.relname
...
export function findIndexRelationships(
  ...
  for (let leftPosition = 0; leftPosition < indexes.length; leftPosition += 1) {
    ...
    for (
      ...
      if (
        !right ||
        left.tableOid !== right.tableOid ||
        left.accessMethod !== right.accessMethod
      )
        continue;
      if ((left.predicate ?? "") !== (right.predicate ?? "")) continue;
      const leftKeys = JSON.stringify(left.keyColumns);
      const rightKeys = JSON.stringify(right.keyColumns);
      if (
        leftKeys === rightKeys &&
        JSON.stringify(left.includedColumns) ===
          JSON.stringify(right.includedColumns)
      ) {
        relationships.push({
          kind: "duplicate",
          tableOid: left.tableOid,
          leftIndexOid: left.indexOid,
          rightIndexOid: right.indexOid,
          leftName: left.name,
          rightName: right.name,
          evidence: `Both indexes use ${left.accessMethod} on (${left.keyColumns.join(", ")})`,
        });
        continue;
      }
      const shorter =
        left.keyColumns.length <= right.keyColumns.length ? left : right;
      const longer = shorter === left ? right : left;
      if (
        shorter.keyColumns.length > 0 &&
        shorter.keyColumns.every(
          (key, position) => longer.keyColumns[position] === key,
        )
      ) {
        relationships.push({
          kind: "prefix",
          tableOid: left.tableOid,
          leftIndexOid: shorter.indexOid,
          rightIndexOid: longer.indexOid,
          leftName: shorter.name,
          rightName: longer.name,
          evidence: `(${shorter.keyColumns.join(", ")}) prefixes (${longer.keyColumns.join(", ")})`,
        });
      }
    }
  }
